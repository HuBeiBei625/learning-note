...
在线服务场景中，请求长度差异明显。例如，同一服务实例可能同时接收 Short Request、中等对话请求和超长文档分析请求。若 Scheduler 直接按照到达顺序组装 batch，Long Request 的 Prefill 可能长时间占用计算资源，导致 Short Request 的 TTFT 升高；若 Scheduler 持续偏向 Short Request 或 Decode Request，Long Request 又可能长期得不到推进，形成 Starvation。
...
本专利的创新点不在于简单地将 Long Prefill Request 切分为 chunk，也不在于普通的 Decode-first 或 Prefill-first 调度策略，而在于提出一种面向混合 Long/Short Requests 的动态 Batch Assembly 决策机制。该机制在每一轮推理迭代中，根据实时负载、请求等待状态、KV Cache 状态和历史执行反馈，动态决定 Decode Request、Short Prefill Request 与 Long Prefill Chunk 的组装比例。

核心创新点可概括如下：

1. **基于 Dynamic Budget 的 Batch Assembly 机制**：将每轮 batch 的 token budget 动态划分为 Decode Guarantee Budget、Short Request TTFT Budget 和 Long Request Progress Budget，而不是使用固定比例或固定阈值进行调度。该机制能够根据服务状态实时调整不同类型请求在 batch 中的资源占比。
2. **Dual-Watermark Feedback Control**：针对 Decode Pressure、Short Request Pressure、Long Request Debt 和 KV Cache Pressure 设计 Low/High Watermark。当某类 pressure 超过 High Watermark 时，自动提高对应 budget；当 pressure 低于 Low Watermark 时，释放对应 budget，从而形成可持续调节的 feedback loop。
3. **Long Request Debt Ledger**：为 Long Request 维护 Debt Value，用于记录其因 Short Request 插队、Decode Guarantee 或 KV Cache 不足而被延迟服务的程度。当 Debt Value 累积到一定程度时，Scheduler 会为 Long Request 保留 Progress Budget，从而避免 Long Request Starvation。
4. **KV Cache-aware Admission 与 Chunk Scheduling**：Batch Assembly 不仅检查本轮 token budget，还联合考虑 KV Cache 可用空间、保守预留长度、Decode 后续增量以及 Long Prefill Chunk 对 KV Cache 的后续影响，降低过度 Admission、Preemption 和 Cache Thrashing。
5. **Dynamic Chunk Size 计算方法**：Long Prefill Chunk Size 由 Long Request Debt、Decode Pressure、KV Cache Pressure 和剩余 Long Request Budget 共同决定，而不是固定 chunk size。该方法能够在短请求延迟、Decode 平滑性、长请求公平性和硬件利用率之间自适应折中。
6. **面向 TTFT、ITL、Throughput 和 Fairness 的闭环优化**：通过 Execution Feedback Collector 持续采集 TTFT、ITL、batch runtime、KV Cache Usage 和 Preemption 次数，并将其反馈给 Budget Controller 和 Debt Ledger，使下一轮 Batch Assembly 具备在线自适应能力。

因此，本专利相对于现有 Chunked Prefill 或 Continuous Batching 的主要区别在于：现有方案通常关注“是否切分 Prefill”或“Decode 与 Prefill 如何交错”，而本方案进一步引入 Dynamic Budget、Dual-Watermark、Debt Ledger 和 KV Cache-aware Constraint 的联合决策，使 batch 组装过程能够根据实时服务状态持续调整。
...
### 6.3 Dual-Watermark Feedback Control
...
### 6.5 KV Cache Joint Constraint

Scheduler 在选择请求进入 batch 时，不仅检查本轮 token budget，还检查以下 KV Cache 条件：

1. 当前 chunk 所需 KV Cache block 是否可分配。
...
## 九、Dynamic Budget Feedback Loop
...
## 十、Key Algorithm Description

本节伪代码与前述三幅图对应：Core Scheduling Flow 对应“算法 1：单轮 Batch Assembly”，System Modules 对应 Decode、Short Prefill、Long Prefill 等子过程，Dynamic Budget Feedback Loop 对应“算法 2：Dynamic Budget Allocation”和“算法 7：Feedback Update”。

### 10.0 结合图示的核心伪代码

以下伪代码采用纯 Markdown fenced code block 形式保存，不包含 LaTeX 伪代码命令，适合在普通 Markdown 阅读器中直接查看。

#### 10.0.1 单轮 Batch Assembly
...
#### 10.0.2 Dynamic Budget Controller
...
#### 10.0.3 Decode Request 组装

**算法 3：Decode Request 组装**
...
#### 10.0.4 Short Prefill Request 组装
...
#### 10.0.6 Long Request Dynamic Chunk Size
